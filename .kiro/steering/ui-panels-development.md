---
inclusion: fileMatch
fileMatchPattern: ['source/panels/**/*.vue', 'source/panels/**/*.ts', 'source/panels/**/*.js']
---
    ...
    <ui-panel-body>
      ...
      <div class="panel-content">
        ...
        <div class="config-section">
          <ui-label>服务器配置</ui-label>
          <ui-input v-model="config.port" placeholder="端口号" />
        </div>

        <!-- 状态显示 -->
        <div class="status-section">
          <ui-label>状态: {{ serverStatus }}</ui-label>
        </div>
      </div>
    </ui-panel-body>
  </ui-panel>
...
### 1. 发送消息到主进程
```typescript
// 方法1: 使用Editor.Message.request (异步，有返回值)
const result = await Editor.Message.request('cocos-mcp', 'start-mcp-server', {
  port: 3000
});

// 方法2: 使用Editor.Message.send (同步，无返回值)
  ...
  setup() {
    ...
    const validateForm = () => {
      ...
      if (!form.value.port || form.value.port < 3000 || form.value.port > 9999) {
        errors.value.port = '端口号必须在3000-9999之间';
      }
    };
