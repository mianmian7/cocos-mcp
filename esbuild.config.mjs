...
const entries = [
    ...
    {
        entryPoints: ['source/panels/image-generator/index.ts'],
        outfile: 'dist/panels/image-generator/index.js',
    },
    // Panel: ai-config
    {
        entryPoints: ['source/panels/ai-config/index.ts'],
        outfile: 'dist/panels/ai-config/index.js',
    },
];

async function build() {
    console.log('🔨 Building Cocos MCP extension with esbuild...\n');
    ...
    // Copy sharp's dependencies (it has peer dependencies)
    const sharpDeps = ['color', 'color-convert', 'color-name', 'color-string', 'detect-libc', 'simple-swizzle', 'is-arrayish', 'semver'];
    for (const dep of sharpDeps) {
        ...
        if (fs.existsSync(srcPath) && !fs.existsSync(destPath)) {
            copyRecursive(srcPath, destPath);
            console.log(`✅ Copied sharp dep: ${dep}`);
        }
    }
    ...
    console.log('  - node_modules/ with native modules (sharp)');
}
