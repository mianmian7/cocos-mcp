  ...
  switch (action) {
    case 'delete_nodes': {
      ...
      for (const uuid of uuids) {
        const decodedUuid = McpServerManager.decodeUuid(uuid);
    ...
    case 'batch_modify': {
      ...
      for (const mod of modifications) {
        const decodedUuid = McpServerManager.decodeUuid(mod.uuid);
...
export function registerApplyGatedActionTool(server: McpServer): void {
  server.registerTool(
    'apply_gated_action',
    {
...
- 某些操作（如 save_scene）风险较低，可直接执行`,
    ...
    async (args) => {
      ...
      if (approvalToken) {
        ...
        if (pending.action !== action) {
          return {
            content: [{
              ...
              text: JSON.stringify({
                success: false,
                error: `Token was issued for '${pending.action}', not '${action}'`
              }, null, 2)
            }],
            isError: true
          };
        ...
        try {
          const result = await executeGatedAction(action, pending.params);
          return {
            content: [{
              type: 'text' as const,
              text: JSON.stringify({
      ...
      if (!actionDef.requiresConfirmation && skipConfirmation) {
        try {
          const result = await executeGatedAction(action, params);
          return {
            content: [{
              type: 'text' as const,
              text: JSON.stringify({
      ...
      const summary = generateActionSummary(action, params);
      const now = Date.now();
