...
import { tryToAddComponent } from "../utils";

export function registerModifyNodesTool(server: McpServer): void {
    ...
    async (args) => {
      ...
      try {
        for (const nodeSpec of nodes) {
          ...
          try {
            const decodedNodeUuid = McpServerManager.decodeUuid(nodeSpec.uuid);
            ...
            if (nodeSpec.removeComponentUuids && nodeSpec.removeComponentUuids.length > 0) {
              for (const componentUuid of nodeSpec.removeComponentUuids) {
                try {
                  const decodedComponentUuid = McpServerManager.decodeUuid(componentUuid);
            ...
            if (nodeSpec.newParentUuid && nodeSpec.newParentUuid.length > 0) {
              try {
                ...
                await Editor.Message.request('scene', 'set-parent', {
                  parent: McpServerManager.decodeUuid(nodeSpec.newParentUuid),
