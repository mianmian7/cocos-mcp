 ...
 * 而不需要为每个 API 创建单独的 MCP 工具。
 */

// Allowlist 定义：哪些操作是允许的
// mode: 'read' = 只读操作（安全），'write' = 写入操作（需谨慎）
interface AllowedCommand {
  channel: string;
  command: string;
  mode: 'read' | 'write';
  description: string;
  argsSchema?: string; // 参数说明
}

const ALLOWED_COMMANDS: AllowedCommand[] = [
  // === Scene 读取操作 ===
  // NOTE: query-node-tree 已移除，使用 get_editor_context 代替（有 maxNodes/maxDepth 限制）
  { channel: 'scene', command: 'query-node', mode: 'read', 
    description: '查询单个节点详情', argsSchema: '[nodeUuid: string]' },
  { channel: 'scene', command: 'query-scene-info', mode: 'read', 
    description: '查询当前场景信息' },
  { channel: 'scene', command: 'query-classes', mode: 'read', 
    description: '查询可用的组件类型' },
  
  // === Scene 写入操作 ===
  { channel: 'scene', command: 'create-node', mode: 'write', 
    description: '创建节点', argsSchema: '[{ parent?: string, assetUuid?: string, type?: string, name?: string }]' },
  { channel: 'scene', command: 'remove-node', mode: 'write', 
    description: '删除节点', argsSchema: '[{ uuid: string }]' },
  { channel: 'scene', command: 'set-property', mode: 'write', 
    description: '设置节点属性', argsSchema: '[{ uuid: string, path: string, dump: { type: string, value: any } }]' },
  { channel: 'scene', command: 'create-component', mode: 'write', 
    description: '添加组件', argsSchema: '[{ uuid: string, component: string }]' },
  { channel: 'scene', command: 'remove-component', mode: 'write', 
    description: '移除组件', argsSchema: '[{ uuid: string, component: string }]' },
  { channel: 'scene', command: 'move-array-element', mode: 'write', 
    description: '移动数组元素（调整顺序）', argsSchema: '[{ uuid: string, path: string, target: number }]' },
  
  // === Scene 操作 ===
  { channel: 'scene', command: 'open-scene', mode: 'write', 
    description: '打开场景', argsSchema: '[sceneAssetUuid: string]' },
  { channel: 'scene', command: 'save-scene', mode: 'write', 
    description: '保存当前场景' },
  
  // === Asset-DB 读取操作 ===
  { channel: 'asset-db', command: 'query-asset-info', mode: 'read', 
    description: '查询资产信息', argsSchema: '[urlOrUuid: string]' },
  { channel: 'asset-db', command: 'query-asset-meta', mode: 'read', 
    description: '查询资产元数据', argsSchema: '[urlOrUuid: string]' },
  { channel: 'asset-db', command: 'query-assets', mode: 'read', 
    description: '批量查询资产', argsSchema: '[{ pattern?: string, type?: string }]' },
  { channel: 'asset-db', command: 'query-path', mode: 'read', 
    description: '查询资产路径', argsSchema: '[uuid: string]' },
  { channel: 'asset-db', command: 'query-url', mode: 'read', 
    description: '查询资产 URL', argsSchema: '[uuid: string]' },
  
  // === Asset-DB 写入操作 ===
  { channel: 'asset-db', command: 'create-asset', mode: 'write', 
    description: '创建资产', argsSchema: '[url: string, content?: string | Buffer]' },
  { channel: 'asset-db', command: 'save-asset-meta', mode: 'write', 
    description: '保存资产元数据', argsSchema: '[urlOrUuid: string, meta: object]' },
  { channel: 'asset-db', command: 'refresh-asset', mode: 'write', 
    description: '刷新资产', argsSchema: '[urlOrUuid: string]' },
  { channel: 'asset-db', command: 'import-asset', mode: 'write', 
    description: '导入资产', argsSchema: '[urlOrUuid: string]' },
  
  // === Selection ===
  { channel: 'selection', command: 'select', mode: 'write', 
    description: '选择节点', argsSchema: '[type: "node" | "asset", uuids: string[]]' },
  { channel: 'selection', command: 'unselect', mode: 'write', 
    description: '取消选择', argsSchema: '[type: "node" | "asset", uuids: string[]]' },
  { channel: 'selection', command: 'clear', mode: 'write', 
    description: '清除选择', argsSchema: '[type: "node" | "asset"]' },
  
  // === Project ===
  { channel: 'project', command: 'query-settings', mode: 'read', 
    description: '查询项目设置', argsSchema: '[protocol: string, key?: string]' },
];

// 构建快速查找表
const allowlistMap = new Map<string, AllowedCommand>();
for (const cmd of ALLOWED_COMMANDS) {
  allowlistMap.set(`${cmd.channel}:${cmd.command}`, cmd);
}

function isAllowed(channel: string, command: string): AllowedCommand | undefined {
  return allowlistMap.get(`${channel}:${command}`);
}
...
function processArgs(args: any[]): any[] {
  ...
  return args.map(arg => {
    if (typeof arg === 'string' && arg.length > 20) {
      // 可能是 UUID，尝试解码
      try {
        return McpServerManager.decodeUuid(arg);
      } catch {
        return arg;
      }
    }
    if (typeof arg === 'object' && arg !== null) {
      // 递归处理对象中的 uuid 字段
      const processed = { ...arg };
      for (const key of ['uuid', 'parent', 'assetUuid', 'nodeUuid']) {
        if (typeof processed[key] === 'string') {
          try {
            processed[key] = McpServerManager.decodeUuid(processed[key]);
          } catch {
            // 保持原值
          }
        }
...
export function registerEditorRequestTool(server: McpServer): void {
  server.registerTool(
    'editor_request',
    {
...
**获取可用命令列表：**
调用时设置 listCommands=true 可获取所有可用命令。`,
      inputSchema: {
    ...
    async (params) => {
      ...
      // 列出可用命令
      if (listCommands) {
        const commandsByChannel: Record<string, { command: string; mode: string; description: string; args?: string }[]> = {};
        for (const cmd of ALLOWED_COMMANDS) {
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              totalCount: ALLOWED_COMMANDS.length
            }, null, 2)
          }]
        };
      }
      ...
      // 检查是否在 allowlist 中
      const allowed = isAllowed(channel, command);
      if (!allowed) {
        return {
          content: [{
            type: 'text' as const,
            text: JSON.stringify({
              ...
              availableChannels: ['scene', 'asset-db', 'selection', 'project']
            }, null, 2)
          }],
          isError: true
        };
...
function encodeUuidsInResult(obj: any, depth = 0): any {
  ...
  if (typeof obj === 'string' && isLikelyUuid(obj)) {
    try {
      return McpServerManager.encodeUuid(obj);
    } catch {
      return obj;
    }
  }
