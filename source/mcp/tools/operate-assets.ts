...
export function registerOperateAssetsTool(server: McpServer): void {
            ...
            switch (operation) {
              case "create": {
                ...
                if (template.url && template.url.length > 0) {
                      ...
                      if (templateInfo && templateInfo.file && fs.existsSync(templateInfo.file)) {
                        ...
                        if (createResult) {
                          result = { 
                            operation: "create", 
                            path: actualPath, 
                            uuid: McpServerManager.encodeUuid(createResult.uuid),
                            ...(fileNameChanged && { 
                  ...
                  if (createResult) {
                    result = { operation: "create", path: finalPath, uuid: McpServerManager.encodeUuid(createResult.uuid) };
                  } else {
                    result = { operation: "create", path: finalPath, success: false };
                  }
                }
              ...
              case "copy": {
                ...
                if (copyResult) {
                  result = { operation: "copy", from: originalAssetPath, to: destinationPath, uuid: McpServerManager.encodeUuid(copyResult.uuid) };
                } else {
              ...
              case "move": {
                ...
                if (moveResult) {
                  result = { operation: "move", from: originalAssetPath, to: destinationPath, uuid: McpServerManager.encodeUuid(moveResult.uuid) };
                } else {
              ...
              case "delete": {
                ...
                result = { operation: "delete", path: originalAssetPath, uuid: McpServerManager.encodeUuid(assetInfo.uuid) };
                break;
              }
