...
import { setProperties, PropertySetSpec, getComponentInfo } from "../utils";

export function registerModifyComponentsTool(server: McpServer): void {
  server.registerTool(
    "modify_components",
    {
      ...
      try {
        ...
        if (components && components.length > 0) {
          for (const componentSpec of components) {
            ...
            try {
              const decodedComponentUuid = McpServerManager.decodeUuid(componentSpec.uuid);
